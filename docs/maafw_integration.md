...
- 使用 `AgentServer.custom_recognition(name)` 注册；
...
`--help` 不加载 Maa runtime。缺少 socket 参数由参数解析器明确拒绝。当前机器若未准备官方 SDK/原生库，只能验证 adapter 与 fake SDK 边界，不能宣称 AgentServer 已实际运行。
