...
视觉功能目前只是离线格点定位基线：尚未接入 MaaFramework，尚不能识别具体事件类型或完整道路关系，也不能可靠确定出口。`current_position` 同时保存人物标记的 `marker_center` 与吸附格点的 `grid_center`，最终节点 `center` 始终等于 `grid_center`。检测结果未经拓扑校验，禁止直接输入求解器。项目不控制游戏、不自动点击，并且没有 GUI。
